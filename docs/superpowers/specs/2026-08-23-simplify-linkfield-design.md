...
|---|---|---|---|
| `linkType` | 50% | `select` | Options: `Linked document` (`reference`) / `Custom URL` (`url`). `defaultValue: 'reference'`, `required: true`. Drives which field row 2 shows. **Naming note:** lexical's own built-in `LinkFeature` already has a base field named `linkType` (values `'internal'`/`'custom'`). Because `LinkField({...}).fields` is spread as an *array* into `LinkFeature({ fields: [...LinkField().fields] })` (see `src/fields/RichText/index.ts`), lexical's `transformExtraFields` **replaces** its base fields with these rather than merging — so there is never a collision at runtime between two different `linkType` fields on the same node. This repo's `linkType` reuses the name deliberately, with `'reference'` standing in for lexical's `'internal'` and `'url'` for its `'custom'`. `creditsLinkValidation.test.ts` previously asserted `linkType` must be *absent* from `LinkField()`'s sanitized fields, as proof the replacement was happening; that assertion is rewritten (see Consumer updates) to assert `linkType` is present with this field's own values instead. |
...
- **`src/lib/unsplash/buildCreditsValue.ts`**: its `linkNode` helper writes `{ reference: null, url, newTab: true, label: text }` — add `linkType: 'url'`. Update its long comment block (enumerates the flat field list and explicitly says "no `linkType` field"; that line is now wrong and must be corrected to describe the replaced-but-reused name).
- **`src/lib/unsplash/buildCreditsValue.test.ts`**: the `fields` equality assertions (`toEqual({ reference: null, url, newTab: true, label })`) need `linkType: 'url'` added to each expected object. The test titled `'emits no linkType field — the caption editor replaces the base link fields'` is rewritten to assert the opposite: every link node's `fields.linkType` equals `'url'`. Rename the test to describe what it now checks (e.g. `'emits LinkField's own linkType — not lexical's default internal/custom values'`).
- **`src/lib/unsplash/creditsLinkValidation.test.ts`**: the sanitized-schema assertion `expect(names).not.toContain('linkType')` is inverted to `expect(names).toContain('linkType')` (proving the replacement happened, now via presence with this field's own values rather than absence). The `arrayContaining([...])` list gains `'linkType'`. The last test in the file (`'a link node that omits label...'`) already passes `linkType: 'custom'` in its input `data` — that literal must become `linkType: 'url'`, since `'custom'` is lexical's own vocabulary and is not one of this field's valid `select` options (an invalid option would now fail sanitized validation instead of passing through). Every other `validateLinkFields(linkFields, {...})` call in this file that omits `linkType` from its `data` object should keep omitting it *only* if the intent is to test the missing-value case; the "reports no validation errors" test's `link.fields` already come from `buildCreditsValue`, so once that emits `linkType: 'url'` this test needs no further change beyond what flows through automatically.
- **`src/lib/unsplash/creditsRendering.test.tsx`**: check for any `fields:` literal enumerating the flat shape and add `linkType: 'url'` if present.
