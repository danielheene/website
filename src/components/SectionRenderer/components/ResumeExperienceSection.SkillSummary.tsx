...
interface ResumeExperienceSectionSkillSummaryProps {
  skillSummary: SkillSummary
  className?: string
}

export const ResumeExperienceSectionSkillSummary = memo(
  function ResumeExperienceSectionSkillSummary({
    skillSummary,
    className,
  }: ResumeExperienceSectionSkillSummaryProps) {
    ...
    const largestSkillTime = useMemo(
      () =>
        Object.values(skillSummary).reduce(
          (prev, { time: curr }) => (curr > prev ? curr : prev),
          0,
        ),
      [skillSummary],
    )
    ...
    const calculateSkillWidth = useCallback(
      (time: number) => `${(time / largestSkillTime) * 100}%`,
      [],
    )
              ...
              <div key={key} className={cn('relative block h-10')}>
                ...
                <div
                  ...
                  style={{
                    ...
                    backgroundImage: `linear-gradient(to right, rgb(var(--color-primary)), rgb(var(--color-primary)) ${width}, rgb(var(--color-white)) ${width}, rgb(var(--color-white)))`,
                  }}
