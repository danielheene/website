...
interface FooterNavGroupsProps extends Pick<PageFooterData, 'navGroups'> {
  className?: string
}
      ...
      {navGroups.map(({ title, entries }, groupKey) => (
        <div key={groupKey}
          ...
          <ul className="text-sm ml-1">
            {entries.map(({ link, id }) => (
              <li key={id} className="font-pp-frama font-medium xl:text-lg text-current/75 hover:text-current">
                <CMSLink {...link} />
              </li>
            ))}
          </ul>
