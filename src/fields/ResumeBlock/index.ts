...
export const ResumeBlockField = <
  K extends string = Extract<BlockSlugValue, `Resume${string}`> & string,
>(props: {
  name: K
   ...
   */
  const images = {
    thumbnail: `/payload/blocks/${dashedName.replaceAll(/(Block|-block)*$/g, '')}-thumbnail.svg`,
    icon: `/payload/blocks/${dashedName.replaceAll(/(Block|-block)*$/g, '')}-icon.svg`,
  }
  ...
  return isComplexLabel
    ...
    : isSimpleLabel
      ...
      : isMissingLabel && hasFallBack
        ? {
            singular: startCase(fallBack)
              .replaceAll(/(Block|-block)*$/g, '')
              .trim(),
            plural: startCase(fallBack)
              .replaceAll(/(Block|-block)*$/g, '')
              .trim(),
          }
        : null
