...
import type { MediaDocument, MediaImage } from '@payload-types'
...
export const generateDocumentThumbnailTask: TaskConfig<'generateDocumentThumbnailTask'> = {
  ...
  handler: async ({ input: { documentId }, req }) => {
    ...
    try {
      ...
      const {
        pages: [screenshot],
      } = await parser.getScreenshot({ first: 1 })
    ...
    try {
      ...
      if (typeof document.thumbnail === 'object' && 'id' in document.thumbnail) {
        previousThumbnail = document.thumbnail
      } else if (typeof document.thumbnail === 'string' && document.thumbnail !== '') {
      ...
      if (previousThumbnail) {
        console.info(`thumbnail for document "${document.filename}" already exists, updating it`)
        documentThumbnail = await req.payload.update({
          collection: CollectionSlug.MediaImages,
          id: previousThumbnail.id,
          data,
          file,
          context: { skipGenerateAlt: true },
          overwriteExistingFiles: true,
        })
        console.info(`updated thumbnail for document "${document.filename}"`)
      } else {
        console.info(`thumbnail for document "${document.filename}" does not exist, creating it`)
      ...
      console.info(`updating document "${document.filename}" with thumbnail id "${documentThumbnail.id}"`)
      await req.payload.update({
        ...
        context: { skipGenerateDocumentThumbnail: true },
      })

      console.info(`updated document "${document.filename}" with thumbnail id "${documentThumbnail.id}" successfully`)
    } catch (error) {
