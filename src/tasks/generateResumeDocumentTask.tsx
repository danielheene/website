...
export const generateResumeDocumentTask: TaskConfig<'generateResumeDocumentTask'> =
  {
    ...
    handler: async ({ input, req }) => {
      ...
      console.info(
        `Checking for existing resume document for locale: ${locale}`,
      )
      ...
      if (typeof documentDownload === 'object' && 'id' in documentDownload) {
        ...
        typeof documentDownload === 'string' &&
        documentDownload !== ''
      ) {
      ...
      try {
        if (existingDocument) {
          ...
          console.info(
            `No existing resume document found for locale: ${locale}, creating new one`,
          )
        ...
        console.error(
          `Failed to create or update resume document: ${fileName}:\n${extractErrorMessage(error)}`,
          error,
        )
